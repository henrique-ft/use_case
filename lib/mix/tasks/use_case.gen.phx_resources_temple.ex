defmodule Mix.Tasks.UseCase.Gen.PhxResourceTemple do
  ...
  defp create_edit_html(context, schema, option_context) do
    path =
      if Keyword.get(option_context, :path, false) do
        "lib/#{context[:web_path]}/templates/#{option_context[:path]}/#{context[:path]}/edit.html.exs"
      else
        "lib/#{context[:web_path]}/templates/#{context[:path]}/edit.html.exs"
      end

    copy_template("temple/edit.html.eex", path,
      schema: schema
    )
  end
  ...
  defp create_form_html(context, schema, option_context) do
    path =
      if Keyword.get(option_context, :path, false) do
        "lib/#{context[:web_path]}/templates/#{option_context[:path]}/#{context[:path]}/form.html.exs"
      else
        "lib/#{context[:web_path]}/templates/#{context[:path]}/form.html.exs"
      end

    copy_template("temple/form.html.eex", path,
      schema: schema,
  ...
  defp create_index_html(context, schema, option_context) do
    path =
      if Keyword.get(option_context, :path, false) do
        "lib/#{context[:web_path]}/templates/#{option_context[:path]}/#{context[:path]}/index.html.exs"
      else
        "lib/#{context[:web_path]}/templates/#{context[:path]}/index.html.exs"
      end

    copy_template("temple/index.html.eex", path,
      schema: schema
    )
  end
  ...
  defp create_new_html(context, schema, option_context) do
    path =
      if Keyword.get(option_context, :path, false) do
        "lib/#{context[:web_path]}/templates/#{option_context[:path]}/#{context[:path]}/new.html.exs"
      else
        "lib/#{context[:web_path]}/templates/#{context[:path]}/new.html.exs"
      end

    copy_template("temple/new.html.eex", path,
      schema: schema
    )
  end
  ...
  defp create_show_html(context, schema, option_context) do
    path =
      if Keyword.get(option_context, :path, false) do
        "lib/#{context[:web_path]}/templates/#{option_context[:path]}/#{context[:path]}/show.html.exs"
      else
        "lib/#{context[:web_path]}/templates/#{context[:path]}/show.html.exs"
      end

    copy_template("temple/show.html.exs", path,
      schema: schema
    )
  end
